...
from dataset import DualImageDataset, get_default_transform_dual, get_validation_transform_dual, dual_image_collate_fn

class DualInputBinaryClassificationLightningModule(pl.LightningModule):
...
class DualInputBinaryClassificationDataModule(pl.LightningDataModule):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def setup(self, stage=None):
        ...
        # Use dual transforms for dual image dataset
        self.train_dataset = DualImageDataset(
            csv_path=self.config['data']['csv_file'],
            root_dir=self.config['data']['root_dir'],
            transform=get_default_transform_dual(image_size=image_size)
        )
        self.val_dataset = DualImageDataset(
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.config['data']['batch_size'], shuffle=True, num_workers=self.config['data']['num_workers'], collate_fn=dual_image_collate_fn)

    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=1, shuffle=False, num_workers=1, collate_fn=dual_image_collate_fn)
