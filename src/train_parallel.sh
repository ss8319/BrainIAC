#!/bin/bash
#SBATCH --job-name=train_brainiac_parallel
#SBATCH --output=logs/train_parallel_%j.out
#SBATCH --error=logs/train_parallel_%j.err
...
# Test GPU availability before running training
echo "=== GPU Test Before Training ==="
$BRAINIAC_PYTHON -c "
import torch
...
echo "Starting Run 3"
$BRAINIAC_PYTHON train_lightning_mci.py --config config_adni_cn_ad_run3.yml &

echo "Starting Run 4"
$BRAINIAC_PYTHON train_lightning_mci.py --config config_adni_cn_ad_run4.yml &
